...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground transition-colors duration-300 selection:bg-(--accent-bg)">
      <Navbar />
      ...
      <main>
        ...
        {/* =========================================
            PROBLEM SECTION
            ========================================= */}
        <section className="max-w-250 mx-auto px-6 md:px-8 py-24 border-t border-(--border-color)">
          <div className="text-center mb-16">
            <span className="inline-block px-4 py-1.5 rounded-full bg-(--accent-bg) border border-(--accent) text-(--accent) text-xs font-bold tracking-wider uppercase mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-(--bg-card) border border-(--border-color) rounded-2xl p-8 text-center">
              ...
              <div className="text-3xl font-bold text-foreground mb-2">15 Min</div>
              <p className="text-sm text-(--text-secondary)">{t('home.problemCard1') as string}</p>
            </div>
            <div className="bg-(--bg-card) border border-(--border-color) rounded-2xl p-8 text-center">
              ...
              <div className="text-3xl font-bold text-foreground mb-2">3 Apps</div>
              <p className="text-sm text-(--text-secondary)">{t('home.problemCard2') as string}</p>
            </div>
            <div className="bg-(--bg-card) border border-(--border-color) rounded-2xl p-8 text-center">
              <div className="w-14 h-14 rounded-2xl bg-red-500/10 flex items-center justify-center mx-auto mb-4">
                <Shield className="w-7 h-7 text-red-500" />
              </div>
              <div className="text-3xl font-bold text-foreground mb-2">₹2.5L</div>
              <p className="text-sm text-(--text-secondary)">{t('home.problemCard3') as string}</p>
            </div>
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={item.num} className="relative">
                <div className="absolute -top-4 -left-4 w-12 h-12 rounded-full bg-(--accent) text-background flex items-center justify-center text-xl font-bold">{item.num}</div>
                <div className="bg-(--bg-card) border border-(--border-color) rounded-2xl p-8 pt-10">
        ...
        {/* =========================================
            ...
            ========================================= */}
        <section className="max-w-350 mx-auto px-6 md:px-8 py-24 border-t border-(--border-color)">
          <div className="text-center mb-12">
            ...
            <p className="text-(--text-secondary)">{t('home.testimonialDesc') as string}</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className="bg-(--bg-card) border border-(--border-color) rounded-2xl p-6 hover:shadow-lg transition-shadow">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 rounded-full bg-(--accent-bg) flex items-center justify-center text-(--accent) font-bold">{tItem.name[0]}</div>
                  <div>
                    <div className="font-bold text-sm">{tItem.name}</div>
                    <div className="text-xs text-(--text-secondary)">{tItem.store}</div>
                  </div>
                </div>
                <blockquote className="text-sm italic mb-4 text-(--text-secondary)">"{tItem.quote}"</blockquote>
                <div className="pt-4 border-t border-(--border-color)">
                  <div className="text-xs text-(--text-secondary)">Result:</div>
                  <div className="font-bold text-(--accent) text-sm">{tItem.result}</div>
                  <div className="text-xs text-(--text-secondary) mt-1">Plan: ClickOut {tItem.plan}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* =========================================
            TRUST BADGES
            ========================================= */}
        <section className="max-w-350 mx-auto px-6 md:px-8 py-16 border-t border-(--border-color)">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {[
              ...
              <div key={i} className="flex items-start gap-3 p-4 rounded-xl bg-(--bg-card) border border-(--border-color)">
        ...
        <section className="max-w-250 mx-auto px-6 md:px-8 py-24 border-t border-(--border-color)">
          <div className="text-center mb-12">
            ...
            <p className="text-(--text-secondary)">{t('home.compareDesc') as string}</p>
          </div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b-2 border-(--border-color)">
                  ...
                  <th className="text-center py-4 px-4 font-bold text-(--accent) bg-(--accent-bg) rounded-t-lg">ClickOut</th>
                  <th className="text-center py-4 px-4 font-semibold text-(--text-secondary)">Khatabook</th>
                  <th className="text-center py-4 px-4 font-semibold text-(--text-secondary)">OkCredit</th>
                  <th className="text-center py-4 px-4 font-semibold text-(--text-secondary)">POSist</th>
                </tr>
              </thead>
              <tbody className="text-(--text-secondary)">
                {[
                  ...
                  <tr key={i} className="border-b border-(--border-color)">
                    <td className="py-4 px-4 font-medium text-foreground">{row[0]}</td>
                    <td className="py-4 px-4 text-center bg-(--accent-bg)/30 font-semibold">{row[1]}</td>
          ...
          <p className="text-xs text-(--text-muted) mt-4 text-center">* Free plans have hidden limits or take commission</p>
        </section>

        {/* =========================================
            FAQ SECTION
            ========================================= */}
        <section id="faq" className="max-w-225 mx-auto px-6 md:px-8 py-24 border-t border-(--border-color)">
          <div className="text-center mb-12">
            <span className="inline-block px-4 py-1.5 rounded-full bg-(--accent-bg) border border-(--accent) text-(--accent) text-xs font-bold tracking-wider uppercase mb-6">
              FAQ
            </span>
            <h2 className="text-3xl md:text-5xl font-bold mb-4 font-modern">{t('home.faqTitle') as string}</h2>
            <p className="text-(--text-secondary)">{t('home.faqDesc') as string}</p>
          </div>
          <div className="space-y-4">
            {faqs.map((faq, idx) => (
              <div key={idx} className="bg-(--bg-card) border border-(--border-color) rounded-2xl overflow-hidden shadow-sm transition-all">
                <button onClick={() => setOpenFaq(openFaq === idx ? null : idx)} className="w-full flex items-center justify-between p-6 text-left gap-4">
                  ...
                  <motion.div animate={{ rotate: openFaq === idx ? 180 : 0 }} className="w-8 h-8 rounded-full bg-background border border-(--border-color) flex items-center justify-center shrink-0">
                    <ChevronDown size={16} className={openFaq === idx ? "text-(--accent)" : "text-(--text-secondary)"} />
                  </motion.div>
                </button>
                <AnimatePresence>
                  {openFaq === idx && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                      <p className="px-6 pb-6 text-(--text-secondary) text-sm md:text-base leading-relaxed">{faq.a}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
        ...
        {/* =========================================
            FINAL CTA SECTION
            ========================================= */}
        <section className="max-w-200 mx-auto px-6 md:px-8 py-24 text-center border-t border-(--border-color)">
          ...
          <p className="text-sm text-(--text-muted) mt-6">{t('home.finalCtaSub') as string}</p>
        </section>
